...
Renderer::GL::Program CreateBatchProgram(const auto& camera)
{
...
int main()
{
...
#if RENDERER_DEBUG
	// debug camera movement controls with wasd
	CameraMovement movement{ camera, *context.GetInputManager() };
#endif

	// Load level data + gameObjects: 	
	...
	// Quad Batcher will render all blocks in a single render call
